import { Box, Paper, Stack, TextField, Typography, List, ListItemButton, ListItemText, Chip } from "@mui/material";
        ...
        <Paper sx={{ p:2 }}>
          ...
          {selected && (
            <Stack spacing={1}>
              ...
              <Typography variant="body2" color="text.secondary">{selected.telephone} · {selected.email || '—'}</Typography>
              <Chip size="small" label={`Historique: ${history.length} réservations`} />
              <Box sx={{ mt:1 }}>
                <Typography fontWeight={700} mb={1}>Historique</Typography>
                {history.map(h => (
                  <Box key={h.id} sx={{ display:'grid', gridTemplateColumns:'1fr 140px 140px', py:0.5 }}>
                    <Box>{h.type}</Box>
