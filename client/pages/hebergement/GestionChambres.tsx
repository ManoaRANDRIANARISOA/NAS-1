                ...
                sx={{
                  ...
                  bgcolor: (() => {
                    const r = reservations.find(
                      (rr) =>
                        rr.type === "hebergement" && rr.chambreId === c.id,
                    );
...
export default function GestionChambres() {
  const list = useMemo(
    () => reservations.filter((r) => r.type === "hebergement"),
    [],
  );
          ...
          <Paper sx={{ p: 2 }}>
            ...
            {list.map((r) => (
              ...
              >
                ...
                <Box>
                  {chambres.find((c) => c.id === r.chambreId)?.numero ?? "-"}
                </Box>
                <Box>{r.statut}</Box>
                <Box>
                  <Button
          ...
          {open && (
            <Stack spacing={1}>
              <Typography fontWeight={700}>
                {open.clientId} ·{" "}
                {chambres.find((c) => c.id === open.chambreId)?.numero}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Arrivée: {format(new Date(open.dateDebut), "dd/MM/yyyy")} ·
                Départ:{" "}
                {open.dateFin
                  ? format(new Date(open.dateFin), "dd/MM/yyyy")
                  : "-"}
              </Typography>
