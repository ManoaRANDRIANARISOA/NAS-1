...
import { useHebergementReservations, useUpdateHebergementReservation, useCreateHebergementReservation } from "@/services/api";
...
function Legend({ color, label }: { color: string; label: string }) {
  return (
    <Stack direction="row" spacing={1} alignItems="center">
      <Box sx={{ width: 10, height: 10, borderRadius: "50%", bgcolor: color, border: "1px solid", borderColor: "divider" }} />
...
function Calendar({ view, dateRef, statusFilter }: { view: View; dateRef: Date; statusFilter: "all" | "libre" | "reservee" | "occupee" | "maintenance" }) {
  ...
  function roomDerivedStatus(roomId: string) {
    ...
    const has = (res || []).some(
      (r) => r.type === "hebergement" && r.chambreId === roomId && isWithinInterval(new Date(r.dateDebut), range),
    );
  ...
  function hasReservation(cId: string, dStart: Date, dEnd: Date) {
    const r = (res || []).find(
      (rr) => rr.type === "hebergement" && rr.chambreId === cId && isWithinInterval(new Date(rr.dateDebut), { start: dStart, end: dEnd }),
    );
    return r;
  }
  ...
  if (view === "month") {
    ...
    return (
      <Box sx={{ minWidth: 160 + days.length * DAY_W }}>
        <Box sx={{ display: "grid", gridTemplateColumns: `160px repeat(${days.length}, ${DAY_W}px)`, gap: 1, px: 1 }}>
          <Box />
          {days.map((d) => (
            <Box key={d.toISOString()} sx={{ textAlign: "center", color: "text.secondary" }}>
              {format(d, "d")}
            </Box>
          ))}
          {rooms.map((c) => (
            <Fragment key={c.id}>
              ...
              {days.map((d, i) => {
                ...
                return (
                  <Box
                    key={`${c.id}-${i}`}
                    sx={{ height: 28, borderRadius: 1, bgcolor: r ? reservationColor(r) : roomStatusColor(c.statut), border: "1px dashed", borderColor: "divider" }}
                  />
                );
              })}
            </Fragment>
  ...
  if (view === "week") {
    ...
    return (
      <Box sx={{ minWidth: 160 + 7 * DAY_W }}>
        <Box sx={{ display: "grid", gridTemplateColumns: `160px repeat(7, ${DAY_W}px)`, gap: 1, px: 1 }}>
          <Box />
          {days.map((d) => (
            <Box key={d.toISOString()} sx={{ textAlign: "center", color: "text.secondary" }}>
              {format(d, "EEE d")}
  ...
  return (
    <Box sx={{ minWidth: 160 + 24 * HOUR_W }}>
      <Box sx={{ display: "grid", gridTemplateColumns: `160px repeat(24, ${HOUR_W}px)`, gap: 1, px: 1 }}>
        <Box />
        {hours.map((h, i) => (
          ...
          <Paper sx={{ p: 2 }}>
            <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
              ...
              <Stack direction="row" spacing={1}>
                ...
                <Button variant="contained" onClick={handleNew}>Nouvelle réservation</Button>
              </Stack>
            </Stack>
            ...
            <EditReservation
              ...
              onSave={(p) => {
                if (String(p.id).startsWith("temp-")) {
                  ...
                  create.mutate(payload as any, { onSuccess: () => setOpen(null) });
                } else {
                  (update as any).mutate(p as any, { onSuccess: () => setOpen(null) });
                }
              }}
            />
          )}
...
function EditReservation({ r, onSave, onClose }: { r: Reservation; onSave: (p: Partial<Reservation> & { id: string }) => void; onClose: () => void }) {
  ...
  const [lines, setLines] = useState<{ label: string; qte: number; prix: number }[]>([{ label: "Nuitée", qte: 1, prix: 120000 }]);
  ...
  return (
    <Stack spacing={1}>
      ...
      <Button variant="outlined" onClick={() => setLines((ls) => [...ls, { label: "Article", qte: 1, prix: 0 }])}>
        Ajouter une ligne
      </Button>
      <Typography>
      ...
      <Stack direction="row" spacing={1}>
        ...
        <Button variant="contained" onClick={() => onSave({ id: r.id, ...form })}>
          Valider
        </Button>
      </Stack>
    </Stack>
