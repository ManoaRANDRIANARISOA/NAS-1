      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "280px 1fr" }, gap: 2 }}>
        <Box>
          <Paper sx={{ p: 2, mb: 2 }}>
            ...
            <TextField size="small" placeholder="Rechercher un utilisateur, un rôle..." fullWidth />
        ...
        <Box>
          ...
          <Paper sx={{ p: 2, mb: 2 }}>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 140px 140px 160px 100px 100px", px: 1, py: 1, color: "text.secondary", fontWeight: 700 }}>
              <Box>Utilisateur</Box><Box>Rôle</Box><Box>Canal</Box><Box>Dernière activité</Box><Box>Statut</Box><Box>Actions</Box>
            </Box>
            {users.map((u) => (
              <Box key={u.id} sx={{ display: "grid", gridTemplateColumns: "1fr 140px 140px 160px 100px 100px", px: 1, py: 1, borderTop: "1px solid", borderColor: "divider", alignItems: "center" }}>
                ...
                <Box>{u.canal}</Box>
                <Box>{u.derniere || "—"}</Box>
                <Box><Chip size="small" label={u.statut} color={u.statut === "Actif" ? "success" : u.statut === "Invité" ? "warning" : "default"} /></Box>
                <Box><Button size="small">Gérer</Button></Box>
              </Box>
            ))}
          </Paper>
          ...
          <Paper sx={{ p: 2, mb: 2 }}>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 100px", rowGap: 1, columnGap: 2 }}>
              <Typography>Accès hébergement</Typography><Switch checked={perm.hebergement} onChange={(e) => setPerm((p) => ({ ...p, hebergement: e.target.checked }))} />
              <Typography>Accès restaurant</Typography><Switch checked={perm.restaurant} onChange={(e) => setPerm((p) => ({ ...p, restaurant: e.target.checked }))} />
              <Typography>Gestion des stocks</Typography><Switch checked={perm.stock} onChange={(e) => setPerm((p) => ({ ...p, stock: e.target.checked }))} />
              <Typography>Facturation</Typography><Switch checked={perm.facturation} onChange={(e) => setPerm((p) => ({ ...p, facturation: e.target.checked }))} />
              <Typography>Rapports</Typography><Switch checked={perm.rapports} onChange={(e) => setPerm((p) => ({ ...p, rapports: e.target.checked }))} />
            </Box>
          </Paper>
          ...
          <Paper sx={{ p: 2 }}>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 120px 120px 120px 120px", px: 1, py: 1, color: "text.secondary", fontWeight: 700 }}>
              <Box>Nom du rôle</Box><Box>Utilisateurs</Box><Box>Hébergement</Box><Box>Restaurant</Box><Box>Facturation</Box>
            </Box>
            {roles.map((r) => (
