...
export default function RestoStock() {
  ...
  const list = useMemo(()=> stockProduits.filter(p => p.famille==='Restaurant' && p.nom.toLowerCase().includes(q.toLowerCase())), [q]);
  const filtered = list.filter(p => view==='all' ? true : view==='low' ? p.stock <= p.seuilMin : p.stock===0);
    ...
    <Box>
      ...
      <Paper sx={{ p:2, mb:2 }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          ...
          <TextField size="small" placeholder="Rechercher un produit" value={q} onChange={e=>setQ(e.target.value)} />
        </Stack>
      </Paper>
      <Paper sx={{ p:2 }}>
        ...
        {filtered.map(p => (
          <Box key={p.id} sx={{ display:'grid', gridTemplateColumns:'1fr 120px 120px', px:1, py:1, borderTop:'1px solid', borderColor:'divider', alignItems:'center' }}>
            <Box>{p.nom}</Box><Box>{p.seuilMin}</Box><Box>{p.stock}</Box>
          </Box>
        ))}
      </Paper>
    </Box>
