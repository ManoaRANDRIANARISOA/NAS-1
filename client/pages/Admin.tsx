...
export default function AdminPage() {
  ...
  const filtered = useMemo(
    () =>
      data.filter((u) =>
        `${u.nom} ${u.email}`.toLowerCase().includes(q.toLowerCase()),
      ),
    [q, data],
  );
  ...
  function saveUser(payload: any) {
    if (drawer?.mode === "create")
      setData((d) => [
        ...d,
        { id: `u${Date.now()}`, derniere: "", ...payload },
      ]);
    else if (drawer?.mode === "edit" && drawer.id)
      setData((d) =>
        d.map((u) => (u.id === drawer.id ? { ...u, ...payload } : u)),
      );
    setDrawer(null);
  }
    ...
    <Box>
      ...
      <Stack direction={{ xs: "column", md: "row" }} spacing={2} sx={{ mb: 2 }}>
        <Paper sx={{ p: 2, flex: 1 }}>
          ...
          <Stack direction="row" spacing={1} sx={{ mt: 1, flexWrap: "wrap" }}>
            ...
            <Chip
              size="small"
              label={`Invités ${invites}`}
              variant="outlined"
            />
            <Chip
              size="small"
              label={`Suspendus ${suspendus}`}
              variant="outlined"
            />
          </Stack>
        </Paper>
        <Paper sx={{ p: 2, width: { xs: "100%", md: 320 } }}>
          ...
          <Stack spacing={1}>
            <Button variant="outlined" onClick={() => onRolePreset("Admin")}>
              Administrateur
            </Button>
            <Button
              variant="outlined"
              onClick={() => onRolePreset("Réception")}
            >
              Réception
            </Button>
            <Button variant="outlined" onClick={() => onRolePreset("Serveur")}>
              Serveur
            </Button>
          </Stack>
        </Paper>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        {filtered.map((u) => (
          ...
          >
            ...
            <Box>
              <Button size="small" onClick={() => openEdit(u.id)}>
                Gérer
              </Button>
            </Box>
          </Box>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <Stack direction="row" spacing={1}>
            <Select
              ...
              onChange={(e) => {
                ...
                applyRoleToPerm(e.target.value as string);
              }}
            >
              {roles.map((r) => (
                <MenuItem key={r.id} value={r.nom}>
                  {r.nom}
                </MenuItem>
              ))}
            </Select>
        ...
        >
          ...
          <Switch
            checked={perm.hebergement}
            onChange={(e) =>
              setPerm((p) => ({ ...p, hebergement: e.target.checked }))
            }
          />
          <Typography>Accès restaurant</Typography>
          <Switch
            checked={perm.restaurant}
            onChange={(e) =>
              setPerm((p) => ({ ...p, restaurant: e.target.checked }))
            }
          />
          ...
          <Switch
            checked={perm.stock}
            onChange={(e) =>
              setPerm((p) => ({ ...p, stock: e.target.checked }))
            }
          />
          <Typography>Facturation</Typography>
          <Switch
            checked={perm.facturation}
            onChange={(e) =>
              setPerm((p) => ({ ...p, facturation: e.target.checked }))
            }
          />
          <Typography>Rapports</Typography>
          <Switch
            checked={perm.rapports}
            onChange={(e) =>
              setPerm((p) => ({ ...p, rapports: e.target.checked }))
            }
          />
        </Box>
      </Paper>
